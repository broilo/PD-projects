...
{    
    ...
    TComplex ws=Power(mush,5)*Power(b,3)*BesselK3(mush*b)/(96.*Pi());  
...
double KerForward(double *x,double *par)
{
    ...
    //Form factors
    /*******************************************************/
    TComplex WSh=WSH(b,W,nu1,nu2);
    double WEvS=WSoft(b,muevsoft); 
    double WOddS=WSoft(b,muoddsoft); 
    /*******************************************************/
    //SigQCD
    /*******************************************************/
    TComplex sigij=SigQCD(W);
    /*******************************************************/
    //Eikonal
    /*******************************************************/
    TComplex ChiEvenSH=0.5*Kf*WSh*sigij;   
    /*******************************************************/
    TComplex ChiEvenSoft=0.5*WEvS*(A+(B*TComplex::Exp(TComplex::I()*PiOver4())/Sqrt(s/s0))+C*TComplex::Power((Y-TComplex::I()*Pi()*0.5),2));     
    /*******************************************************/
    TComplex ChiOddSoft=0.5*WOddS*D*TComplex::Exp(-TComplex::I()*PiOver4())/Sqrt(s/s0);     
    /*******************************************************/
    TComplex ChiEven=ChiEvenSH+ChiEvenSoft;
    double ReChiEven=ChiEven.Re();
    double ReChiOdd=ChiOddSoft.Re(); 
    double ImChiEven=ChiEven.Im();
    double ImChiOdd=ChiOddSoft.Im();        
...
// Plotting St data     
    ...
    grSigPBARP_CT14->Draw("csame");
      ...
      canv1->SaveAs("St_DGM19cCT14cut7vFF_testeV2.eps");    
//       canv1->Clear();
}
/*******************************************************************************/
...
//       c1->SetGrid();
    ...
    grRhoPBARP_CT14->Draw("csame");
      ...
      canv2->SaveAs("Rh_DGM19cCT14cut7vFF_testeV2.eps");      
      canv2->Clear();
}
/***********************************************************************************************************/
...
void DGM19cCT14cut7vFF_testeV2()
{
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_DGM19cCT14cut7vFF_testeV2.dat","w"); 
    aq2=fopen("Rhpp_DGM19cCT14cut7vFF_testeV2.dat","w"); 
    aq3=fopen("Stpa_DGM19cCT14cut7vFF_testeV2.dat","w"); 
    aq4=fopen("Rhpa_DGM19cCT14cut7vFF_testeV2.dat","w");  
    ...
    double sigpp,rhopp,sigpbarp,rhopbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_DGM19cCT14cut7vFF_testeV2.dat","r");
      aq2 = fopen("Rhpp_DGM19cCT14cut7vFF_testeV2.dat","r");
      aq3 = fopen("Stpa_DGM19cCT14cut7vFF_testeV2.dat","r");
      aq4 = fopen("Rhpa_DGM19cCT14cut7vFF_testeV2.dat","r");
