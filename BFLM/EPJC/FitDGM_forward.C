//D.A. Fagundes, 09.11.2018 (última atualização)
#include <chrono> 
...
/*******************************************************************************/
//QCD cross section - Mateus' analytical parametrization for Cut13 PDF
/*******************************************************************************/
...
double KerForward(double *x,double *par)
{
    ...
    /*******************************************************/
    double ReChiOddSoft=0.5*WOddS*(D*Cos(PiOver4())/Sqrt(s/s0)+E*Pi()*Y);     
    /*******************************************************/
    ...
    /*******************************************************/
    double ImChiOddSoft=0.5*WOddS*(-D*Sin(PiOver4())/Sqrt(s/s0)+E*(Y2-Pi()*PiOver4()));
    /*******************************************************/    
...
void PlotTotXSec(double *Wcm,double *sigtotPPCT14,double *sigtotPBARPCT14,int npfit)
{
    ...
    aq1 = fopen("../data/py_Stpac10GeV.dat","r");
    aq2 = fopen("../data/py_StppTAc10GeV.dat","r");
    ...
    for(int i=0;i<npdpa;i++)     
     ...
     fclose(aq2);
      ...
      leg1->AddEntry(grSigPP_CT14,"CT14 - pp","l");
      leg1->AddEntry(grSigPBARP_CT14,"CT14 - #bar{p}p","l");
      leg1->SetFillColor(0);
      ...
      canv1->SaveAs("TotXSec_CT14_Cut13_Odderon.eps");    
//       canv1->Clear();
}
...
void PlotRho(double *Wcm,double *rhoPPCT14,double *rhoPBARPCT14,int npfit)
{
  ...
  //Plotting rho data        
    aq1 = fopen("../data/py_Rhpa.dat","r");
    aq2 = fopen("../data/py_Rhppc10GeV.dat","r");
...
//       c1->SetGrid();
      ...
      leg2->AddEntry(grRhoPP_CT14,"CT14 - pp","l");
      leg2->AddEntry(grRhoPBARP_CT14,"CT14 - #bar{p}p","l");
      leg2->SetFillColor(0);
      ...
      /*******************************************************************************/
        ...
        TPad *pad1 = new TPad("pad1","This is pad1",0.55,0.15,0.88,0.5);
      pad1->SetFillColor(0);
      pad1->SetLogx();
      pad1->Draw();     
      pad1->cd();
      ...
      canv2->SaveAs("Rho_CT14_Cut13_Odderon.eps");      
      canv2->Clear();
}
/***********************************************************************************************************/
...
{    
   /*******************************************************/ 
    ...
    TF1 SigTotPP("SigTotPP",KerForward,bmin,bmax,11);
    ...
    SigTotPP.SetParameter(6,E);
    SigTotPP.SetParameter(7,nu1);
    SigTotPP.SetParameter(8,nu2);
    SigTotPP.SetParameter(9,1);
    SigTotPP.SetParameter(10,1); 
    /*******************************************************/
    ...
    SigTotPBARP.SetParameter(6,E);
    SigTotPBARP.SetParameter(7,nu1);
    SigTotPBARP.SetParameter(8,nu2);
    SigTotPBARP.SetParameter(9,2);
    SigTotPBARP.SetParameter(10,1); 
    /*******************************************************/
    //Rho - pp  
    ...
    RhoPP.SetParameter(6,E);
    RhoPP.SetParameter(7,nu1);
    RhoPP.SetParameter(8,nu2);
    RhoPP.SetParameter(9,1);
    RhoPP.SetParameter(10,2);  
     /*******************************************************/
    //Rho - pbarp  
    ...
    RhoPBARP.SetParameter(6,E);
    RhoPBARP.SetParameter(7,nu1);
    RhoPBARP.SetParameter(8,nu2);
    RhoPBARP.SetParameter(9,2);
    RhoPBARP.SetParameter(10,2);  
    /*******************************************************/
...
void FitDGM_forward()
{
    ...
    double vstart[numpar] = {9.00000e-01,
1.24521e+02,
3.94324e+01,
5.91741e-01,
2.42491e+01,
.1,
1.76372e+00,
1.88560e-02}; //start values
    double step[numpar] = {1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3}; //steps
    ...
    gMinuit->mnparm(5, "E", vstart[5], step[5], 0,0,ierflg);
    gMinuit->mnparm(6, "Nu1", vstart[6], step[6], 0,0,ierflg);
    gMinuit->mnparm(7, "Nu2", vstart[7], step[7], 0,0,ierflg);
...
//     gMinuit->FixParameter(7);
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("TotXSecPP_DGM_Cut13_Odderon.dat","w"); 
    aq2=fopen("RhoPP_DGM_Cut13_Odderon.dat","w"); 
    aq3=fopen("TotXSecPBARP_DGM_Cut13_Odderon.dat","w"); 
    aq4=fopen("RhoPBARP_DGM_Cut13_Odderon.dat","w");  
    ...
    double sigpp,rhopp,sigpbarp,rhopbarp;
     ...
     do{
      ...
      aq1 = fopen("TotXSecPP_DGM_Cut13_Odderon.dat","r");
      aq2 = fopen("RhoPP_DGM_Cut13_Odderon.dat","r");
      aq3 = fopen("TotXSecPBARP_DGM_Cut13_Odderon.dat","r");
      aq4 = fopen("RhoPBARP_DGM_Cut13_Odderon.dat","r");
