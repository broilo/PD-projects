...
/*******************************************************************************/
//QCD cross section - Mateus' analytical parametrization for MMHTLO PDF
/*******************************************************************************/
...
void PlotTotXSec(double *Wcm,double *sigtotPPMMHT,double *sigtotPBARPMMHT,int npfit)
{
    FILE *aq1,*aq2;
    
    aq1 = fopen("paw_Stpa_TAcut5.dat","r");
    aq2 = fopen("paw_Stpp_TAcut5.dat","r");
    ...
    for(int i=0;i<npdpa;i++)     
     ...
     fclose(aq2);
      ...
      leg1->AddEntry(grSigPP_MMHT,"MMHT - pp","l");
      leg1->AddEntry(grSigPBARP_MMHT,"MMHT - #bar{p}p","l");
      leg1->SetFillColor(0);
      ...
      canv1->SaveAs("St_DGM18cMMHTcut13v2_TAcut5.eps");    
//       canv1->Clear();
}

void PlotRho(double *Wcm,double *rhoPPMMHT,double *rhoPBARPMMHT,int npfit)
{
    FILE *aq1,*aq2;
    
  //Plotting rho data        
    aq1 = fopen("paw_Rhpa_TAcut5.dat","r");
    aq2 = fopen("paw_Rhpp_TAcut5.dat","r");
...
//       c1->SetGrid();
      ...
      grRhoPP_MMHT->SetLineColor(1);
      grRhoPP_MMHT->SetLineWidth(1);
      grRhoPP_MMHT->SetLineStyle(1);
      grRhoPP_MMHT->Draw("c"); 
      
      TGraph *grRhoPBARP_MMHT = new TGraph(npfit,Wcm,rhoPBARPMMHT);
      grRhoPBARP_MMHT->SetLineColor(1);
      grRhoPBARP_MMHT->SetLineWidth(1);
      grRhoPBARP_MMHT->SetLineStyle(2);
      ...
      leg2->AddEntry(grRhoPP_MMHT,"MMHT - pp","l");
      leg2->AddEntry(grRhoPBARP_MMHT,"MMHT - #bar{p}p","l");
      leg2->SetFillColor(0);
      ...
      canv2->SaveAs("Rh_DGM18cMMHTcut13v2_TAcut5.eps");      
      canv2->Clear();
}
/***********************************************************************************************************/
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_DGM18cMMHTcut13v2_TAcut5.dat","w"); 
    aq2=fopen("Rhpp_DGM18cMMHTcut13v2_TAcut5.dat","w"); 
    aq3=fopen("Stpa_DGM18cMMHTcut13v2_TAcut5.dat","w"); 
    aq4=fopen("Rhpa_DGM18cMMHTcut13v2_TAcut5.dat","w");  
    ...
    double sigpp,rhopp,sigpbarp,rhopbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_DGM18cMMHTcut13v2_TAcut5.dat","r");
      aq2 = fopen("Rhpp_DGM18cMMHTcut13v2_TAcut5.dat","r");
      aq3 = fopen("Stpa_DGM18cMMHTcut13v2_TAcut5.dat","r");
      aq4 = fopen("Rhpa_DGM18cMMHTcut13v2_TAcut5.dat","r");
      
      const int npfit=108;
      double Wcm[npfit],sigtotPPMMHT[npfit],rhoPPMMHT[npfit],sigtotPBARPMMHT[npfit],rhoPBARPMMHT[npfit];       
            
      for(int j=0;j<npfit;j++){
          fscanf(aq1,"%lg %lg",&Wcm[j],&sigtotPPMMHT[j]);
          fscanf(aq2,"%lg %lg",&Wcm[j],&rhoPPMMHT[j]);
          fscanf(aq3,"%lg %lg",&Wcm[j],&sigtotPBARPMMHT[j]);
          fscanf(aq4,"%lg %lg",&Wcm[j],&rhoPBARPMMHT[j]);
      }
    ...
    PlotTotXSec(Wcm,sigtotPPMMHT,sigtotPBARPMMHT,npfit);      
    /************************************************************************************************/
    //Plotting rho data     
     PlotRho(Wcm,rhoPPMMHT,rhoPBARPMMHT,npfit);     
