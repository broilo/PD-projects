...
c o DGM antigo, foi trocado log2(s/s0) por potência de s (Pomeron simples).
c
...
double KerForward(double *x,double *par)
{
    ...
    /*******************************************************/
    double ReChiEvenSoft=0.5*WEvS*(A+(B*Cos(PiOver4())/Sqrt(s/s0))+C*Power((s/s0),epln)*Cos(epln*PiOver2()));     
    /*******************************************************/
    ...
    /*******************************************************/
    double ImChiEvenSoft=0.5*WEvS*((B*Sin(PiOver4())/Sqrt(s/s0))-C*Power((s/s0),epln)*Sin(epln*PiOver2()));
    /*******************************************************/
...
// Plotting St data     
    ...
    grSigPBARP_CT14->Draw("csame");
      ...
      canv1->SaveAs("St_DGM19cCT14cut7_teste002.eps");    
//       canv1->Clear();
}
/*******************************************************************************/
...
//       c1->SetGrid();
    ...
    grRhoPBARP_CT14->Draw("csame");
      ...
      canv2->SaveAs("Rh_DGM19cCT14cut7_teste002.eps");      
      canv2->Clear();
}
/***********************************************************************************************************/
...
{    
   /*******************************************************/ 
    ...
    SigTotPBARP.SetParameter(7,epln);
...
void DGM19cCT14cut7_teste002()
{
    ...
    0.05414270,
    0.895}; //start values
    double step[numpar] = {1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-2}; //stepss
    ...
    gMinuit->mnparm(7, "epln", vstart[7], step[7], 0,0,ierflg);
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_DGM19cCT14cut7_teste002.dat","w"); 
    aq2=fopen("Rhpp_DGM19cCT14cut7_teste002.dat","w"); 
    aq3=fopen("Stpa_DGM19cCT14cut7_teste002.dat","w"); 
    aq4=fopen("Rhpa_DGM19cCT14cut7_teste002.dat","w");  
    ...
    double sigpp,rhopp,sigpbarp,rhopbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_DGM19cCT14cut7_teste002.dat","r");
      aq2 = fopen("Rhpp_DGM19cCT14cut7_teste002.dat","r");
      aq3 = fopen("Stpa_DGM19cCT14cut7_teste002.dat","r");
      aq4 = fopen("Rhpa_DGM19cCT14cut7_teste002.dat","r");
