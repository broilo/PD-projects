...
double KerForward(double *x,double *par)
{
    ...
    /*******************************************************/
    double ReChiEvenSoft=0.5*WEvS*(A+(B*Cos(PiOver4())/Sqrt(s/s0))+C*Power((s/s0),eps)*Cos(Pi()*eps/2.));     
    /*******************************************************/
    ...
    /*******************************************************/
    double ImChiEvenSoft=0.5*WEvS*((B*Sin(PiOver4())/Sqrt(s/s0))-C*Power((s/s0),eps)*Sin(Pi()*eps/2.));
    /*******************************************************/
...
// Plotting St data     
    ...
    grSigPBARP_CT14->Draw("csame");
      ...
      canv1->SaveAs("St_DGM19cCT14cut7_teste001.eps");    
//       canv1->Clear();
}
/*******************************************************************************/
...
//       c1->SetGrid();
    ...
    grRhoPBARP_CT14->Draw("csame");
      ...
      canv2->SaveAs("Rh_DGM19cCT14cut7_teste001.eps");      
      canv2->Clear();
}
/***********************************************************************************************************/
...
{    
   /*******************************************************/ 
    ...
    SigTotPBARP.SetParameter(7,eps);
...
void DGM19cCT14cut7_teste001()
{
    ...
    2.29757000,
    0.05414270,
    0.89500}; //start values
    double step[numpar] = {1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-3,1.e-2}; //steps
    ...
    gMinuit->mnparm(7, "eps", vstart[7], step[7], 0,0,ierflg);
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_DGM19cCT14cut7_teste001.dat","w"); 
    aq2=fopen("Rhpp_DGM19cCT14cut7_teste001.dat","w"); 
    aq3=fopen("Stpa_DGM19cCT14cut7_teste001.dat","w"); 
    aq4=fopen("Rhpa_DGM19cCT14cut7_teste001.dat","w");  
    ...
    double sigpp,rhopp,sigpbarp,rhopbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_DGM19cCT14cut7_teste001.dat","r");
      aq2 = fopen("Rhpp_DGM19cCT14cut7_teste001.dat","r");
      aq3 = fopen("Stpa_DGM19cCT14cut7_teste001.dat","r");
      aq4 = fopen("Rhpa_DGM19cCT14cut7_teste001.dat","r");
