...
------------------------------------------------------------------------
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20mod_w002_teste.C | cat > BSG20mod_w002_teste.min
   ...
   Last update: 28/fev/2020 (Pelotas, RS, Brazil)
...
double SigPQCDlow(double W)
{
   double s=W*W;
   double sLOW=s-0.01*s;
...
double KerSigUPpp(double *x,double *par)
{
    double b=x[0];      
    double ww=par[0]; 
    double Sig0=par[1];
    ...
    //SigPQCD
    /*******************************************************/
    double SigMinJet=B*SigPQCDup(W)*mbfactor;
    /*******************************************************/
    //SigSoft
    /*******************************************************/
    double SigR1=A1*Power(sUP/s0,-del1);
    double SigR2=A2*Power(sUP/s0,-del2);
    //pp
	...
	/*******************************************************/
            SigUPpp=2.*Pi()*b*(2.*(1.-Power(xx,winv)*CHF))*fm2_to_mb; //in mb 
            return SigUPpp;    
}

double KerSigLOWpp(double *x,double *par)
{
    double b=x[0];      
    double ww=par[0]; 
    double Sig0=par[1];
    ...
    //SigPQCD
    /*******************************************************/
    double SigMinJet=B*SigPQCDlow(W)*mbfactor;
    /*******************************************************/
    //SigSoft
    /*******************************************************/
    double SigR1=A1*Power(sLOW/s0,-del1);
    double SigR2=A2*Power(sLOW/s0,-del2);
    //pp
    double SigSoftPP=(Sig0+SigR1-SigR2);
    /*******************************************************/
    //SigEik
	...
	CHF=gsl_sf_hyperg_U(winv,1.0,xx);
	/*******************************************************/
            SigLOWpp=2.*Pi()*b*(2.*(1.-Power(xx,winv)*CHF))*fm2_to_mb; //in mb 
            return SigLOWpp;    
}

double KerSigUPpbp(double *x,double *par)
{
    double b=x[0];      
    double ww=par[0]; 
    double Sig0=par[1];
	...
	CHF=gsl_sf_hyperg_U(winv,1.0,xx);
	/*******************************************************/
            SigUPpbp=2.*Pi()*b*(2.*(1.-Power(xx,winv)*CHF))*fm2_to_mb; //in mb 
            return SigUPpbp;    
}

double KerSigLOWpbp(double *x,double *par)
{
    double b=x[0];      
    double ww=par[0]; 
    double Sig0=par[1];
	...
	CHF=gsl_sf_hyperg_U(winv,1.0,xx);
	/*******************************************************/
            SigLOWpbp=2.*Pi()*b*(2.*(1.-Power(xx,winv)*CHF))*fm2_to_mb; //in mb 
            return SigLOWpbp;    
}
...
double KerForward(double *x,double *par)
{
    ...
    TF1 SigUPpp("SigUPpp",KerSigUPpp,bmin,bmax,8)
    SigUPpp.SetParameter(0,ww)
    SigUPpp.SetParameter(1,Sig0)
    SigUPpp.SetParameter(2,A1)
    SigUPpp.SetParameter(3,del1)
    SigUPpp.SetParameter(4,A2)
    SigUPpp.SetParameter(5,del2)
    SigUPpp.SetParameter(6,B)
    SigUPpp.SetParameter(7,W)
    
    TF1 SigLOWpp("SigLOWpp",KerSigLOWpp,bmin,bmax,8)
    SigLOWpp.SetParameter(0,ww)
    SigLOWpp.SetParameter(1,Sig0)
    SigLOWpp.SetParameter(2,A1)
    SigLOWpp.SetParameter(3,del1)
    SigLOWpp.SetParameter(4,A2)
    SigLOWpp.SetParameter(5,del2)
    SigLOWpp.SetParameter(6,B)
    SigLOWpp.SetParameter(7,W)
   
    TF1 SigUPpbp("SigUPpbp",KerSigUPpbp,bmin,bmax,8)
    SigUPpbp.SetParameter(0,ww)
    SigUPpbp.SetParameter(1,Sig0)
    SigUPpbp.SetParameter(2,A1)
    SigUPpbp.SetParameter(3,del1)
    SigUPpbp.SetParameter(4,A2)
    SigUPpbp.SetParameter(5,del2)
    SigUPpbp.SetParameter(6,B)
    SigUPpbp.SetParameter(7,W)
    
    TF1 SigLOWpbp("SigLOWpbp",KerSigLOWpbp,bmin,bmax,8)
    SigLOWpbp.SetParameter(0,ww)
    SigLOWpbp.SetParameter(1,Sig0)
    SigLOWpbp.SetParameter(2,A1)
    SigLOWpbp.SetParameter(3,del1)
    SigLOWpbp.SetParameter(4,A2)
    SigLOWpbp.SetParameter(5,del2)
    SigLOWpbp.SetParameter(6,B)
    SigLOWpbp.SetParameter(7,W)
    
    double DerSigPP = (SigUPpp.Eval(b)-SigLOWpp.Eval(b))/sDif;
    double DerSigPBP = (SigUPpbp.Eval(b)-SigLOWpbp.Eval(b))/sDif;
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("St_BSG20mod_w002_teste.eps");    
}
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_BSG20mod_w002_teste.dat","w"); 
    aq2=fopen("Stpbp_BSG20mod_w002_teste.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_BSG20mod_w002_teste.dat","r");
      aq2 = fopen("Stpbp_BSG20mod_w002_teste.dat","r");
