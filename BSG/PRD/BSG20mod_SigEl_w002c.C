 ...
 *** Bandas de incerteza ***
 ...
 Estratégia 2 -> Manter os parâmetros nos valores ajustados e estudar a incerteza em w.
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20mod_SigEl_w002c.C | cat > BSG20mod_SigEl_w002c.min
...
double KerForward(double *x,double *par)
{
	...
	/*******************************************************/
            KerSig=2.*Pi()*b*Power((1.-Power(xx,winv)*CHF),2.)*fm2_to_mb; //in mb 
            return KerSig;    
}
...
// Plotting St data     
    ...
    for(int i=0;i<npdpa;i++)     
     ...
     fclose(aq4);
      ...
      TCanvas *canv1 = new TCanvas("c1","Total Elastic Cross Section",200,10,900,600);
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20mod_SigEl_w002c.eps");    
}
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_BSG20mod_SigEl_w002c.dat","w"); 
    aq2=fopen("Stpbp_BSG20mod_SigEl_w002c.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_BSG20mod_SigEl_w002c.dat","r");
      aq2 = fopen("Stpbp_BSG20mod_SigEl_w002c.dat","r");
