...
------------------------------------------------------------------------
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20mod_w002_minK3s.C | cat > BSG20mod_w002_minK3s.min
...
//Form factors
//----------------------------------------------------------------------
double Whard(double b,double W,double nu1,double nu2)
{    
    double s=W*W;
    double Y=Log(s/s0);
    nu1=(1.7*0.19);
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("St_BSG20mod_w002_minK3s.eps");    
}
//----------------------------------------------------------------------
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("Stpp_BSG20mod_w002_minK3s.dat","w"); 
    aq2=fopen("Stpbp_BSG20mod_w002_minK3s.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("Stpp_BSG20mod_w002_minK3s.dat","r");
      aq2 = fopen("Stpbp_BSG20mod_w002_minK3s.dat","r");
