...
px2,py2 = np.loadtxt("XSecTotPPsoft_BSG20mod_changes.dat", usecols=(0,1), unpack=True)
...
plt.plot(px1,py1, 'r-', linewidth=1.25,label=r"$\sigma^{pp}_{tot}$",zorder=2)
plt.plot(px2,py2, 'b-.', linewidth=1.25,label=r"$\sigma^{pp}_{soft}$",zorder=3)

plt.plot(px3,py3, 'k--', linewidth=1.25,label=r"$\sigma_{pQCD}$",zorder=3)
...
plt.savefig("XSecTotPP_softhard_BSG20mod_changes.eps")
#plt.show()
