 ...
 *** Bandas de incerteza ***
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20mod_V002zb_teste.C | cat > BSG20mod_V002zb_teste.min
...
double KerForward(double *x,double *par)
{
    ...
    double REFminus=k+(Power(E,2.)/16.)*(subDerStPZ);
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("St_BSG20mod_V002zb_teste.eps");    
}
...
*/           
      canv2->SaveAs("Rh_BSG20mod_V002zb_teste.eps");      
      canv2->Clear();
}
//----------------------------------------------------------------------
...
{    
   /*******************************************************/ 
    ...
    TF1 RhoPP("RhoPP",KerForward,Wmin,Wmax,10);
    ...
    TF1 RhoPBARP("RhoPBARP",KerForward,Wmin,Wmax,10);
...
//       cout << Ecm << "\t" << Sigminus + 1. << "\t" << 0.05*Sigminus << "\t" << 0. << endl;
       ...
       //cout << Ecm << "\t" << sigpbarp << "\t" << 0.01*sigpbarp << "\t" << 0. << endl;
