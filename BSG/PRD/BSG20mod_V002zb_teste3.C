 ...
 *** Bandas de incerteza ***
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20mod_V002zb_teste3.C | cat > BSG20mod_V002zb_teste3.min
...
/*
   1  b1           4.78943e+01   2.44327e-01   0.00000e+00   0.00000e+00
   2  b2          -3.86250e+00   4.52855e-02  -0.00000e+00   3.51219e-09
   3  b3           3.78476e-01   1.78526e-03   0.00000e+00  -2.77649e-08
*/
const double b1pp=4.78943e+01;
const double b2pp=-3.86250e+00;
const double b3pp=3.78476e-01;
// Parametrization Stpbp parameters
/*
   1  b1           5.87884e+01   2.71173e-01   1.76014e-09   0.00000e+00
   2  b2          -5.33701e+00   4.85064e-02   3.01531e-11   1.98898e-09
   3  b3           4.23135e-01   1.87047e-03  -7.52266e-13   1.69337e-07
*/
const double b1pbp=5.87884e+01;
const double b2pbp=-5.33701e+00;
const double b3pbp=4.23135e-01;
//----------------------------------------------------------------------
/*
                              Analysis 
*/
...
double StppPZ(double Ener)
{
   double E=Ener*Ener;
    ...
    double sig = b1pp + b2pp*Y + b3pp*Y*Y;
...
{    
   double E=Ener*Ener;
    ...
    double dersig = b2pp/E + 2.*b3pp*Y/E;
...
double StpbpPZ(double Ener)
{
   double E=Ener*Ener;
    ...
    double sig = b1pbp + b2pbp*Y + b3pbp*Y*Y;
...
{    
   double E=Ener*Ener;
    ...
    double dersig = b2pbp/E + 2.*b3pbp*Y/E;
...
double KerForward2(double *x,double *par)
{
    ...
    double ReFplusDivE=k/E+(1./16.)*(-(addStPZ)/E + addDerStPZ);
    double REFminusDivE=k/E+(1./16.)*(subDerStPZ);
    ...
    double KerRho;
    	...
    	//Average number of interactions
    	/*******************************************************/
	ReFDivE=ReFplusDivE+REFminusDivE;
	StDivE=StppPZ(Ener)/E;
    	}
    	else if(reac==2) //pbp
    	{
	ReFDivE=ReFplusDivE-REFminusDivE;
	StDivE=StpbpPZ(Ener)/E;
    	}
    	else
    	{
    	ReFDivE=0.;
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("St_BSG20mod_V002zb_teste3.eps");    
}
...
*/           
      canv2->SaveAs("Rh_BSG20mod_V002zb_teste3.eps");      
      canv2->Clear();
}
//----------------------------------------------------------------------
