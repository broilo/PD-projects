...
------------------------------------------------------------------------
 ...
 Obs3: PDF: CT14
 Obs4: Free: Sig0, A1, A2, B  -> Fixados no ajuste de BSGorig_band1str2_V001.C
 Obs5: w is fixed in 1.0

 *** Bandas de incerteza ***
 
 Estratégia 2 -> Refazer o ajuste fixando B. 
 Intervalo B=[(0.5),1.5]
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_band1str2_V001.C | cat > BSG20orig_SigEl_band1str2_V001.min
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_band1str2_V001.eps");    
}
//----------------------------------------------------------------------
...
void BSG20orig_SigEl_band1str2_V001()
{
    /******************************************/
    //time control - start
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    //fit parameters
    double ww=1.0;//outpar[0]; 
    double Sig0=7.49959e+01;//outpar[0];
    double A1=1.06745e+01;//outpar[1];
    double del1=0.5;//outpar[3];
    double A2=1.70969e+01;//outpar[2];  
    double del2=0.5;//outpar[5];
    double B=0.5;//outpar[3];
    /*
    1  Sig0         7.49959e+01   2.77097e-01  -0.00000e+00   4.37080e-08
    2  A1           1.06745e+01   6.43430e-01   0.00000e+00  -5.32979e-08
    3  A2           1.70969e+01   3.55372e-01   0.00000e+00  -4.08302e-09
    */    
    /*******************************************************/
    //Total XSection - pp
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_band1str2_V001.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_band1str2_V001.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_band1str2_V001.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_band1str2_V001.dat","r");
