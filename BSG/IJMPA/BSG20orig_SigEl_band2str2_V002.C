...
------------------------------------------------------------------------
 ...
 Obs3: PDF: CT14
 Obs4: Free: w, Sig0, A1, A2 -> Fixados no ajuste de BSGorig_band2str2_V002.C
 Obs5: B is fixed in 0.76

 *** Bandas de incerteza ***
 
 Estratégia 2 -> Refazer o ajuste fixando w. 
 Intervalo w=[0.8,(1.6)]
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_band2str2_V002.C | cat > BSG20orig_SigEl_band2str2_V002.min
   ...
   Last update: 5/fev/2020 (Pelotas, RS, Brazil)
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_band2str2_V002.eps");    
}
//----------------------------------------------------------------------
...
void BSG20orig_SigEl_band2str2_V002()
{
    /******************************************/
    //time control - start
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    /*******************************************************/ 
    //fit parameters
    double ww=1.6;//outpar[0]; 
    double Sig0=5.16814e+01;//outpar[0];
    double A1=1.15334e+01;//outpar[1];
    double del1=0.5;//outpar[3];
    double A2=1.32061e+01;//outpar[2];  
    double del2=0.5;//outpar[5];
    double B=0.76;//outpar[3];
    /*
   1  Sig0         5.16814e+01   2.11813e-01   2.27419e-07   7.37848e-06
   2  A1           1.15334e+01   4.93843e-01  -5.78479e-07  -7.23230e-06
   3  A2           1.32061e+01   2.72662e-01  -1.22410e-07  -8.59126e-07
    */    
    /*******************************************************/
    //Total XSection - pp
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_band2str2_V002.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_band2str2_V002.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_band2str2_V002.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_band2str2_V002.dat","r");
