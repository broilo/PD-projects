...
------------------------------------------------------------------------
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_V002.C | cat > BSG20orig_SigEl_V002.min
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_V002.eps");    
}
//----------------------------------------------------------------------
...
{    
   /*******************************************************/ 
    //fit parameters
    double ww=1.31867e+00;//par[0]; 
    double Sig0=5.98107e+01;//par[0];
    double A1=5.69369e+01;//par[1];
    double del1=0.5;//par[3];
    double A2=7.28361e+01;//par[2];  
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    double Sig0=7.20448e+01;//outpar[0];
    double A1=1.29595e+01;//outpar[1];
    double del1=0.5;//outpar[3];
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_V002.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_V002.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_V002.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_V002.dat","r");
