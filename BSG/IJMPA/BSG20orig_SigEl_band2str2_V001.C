...
------------------------------------------------------------------------
 ...
 Obs3: PDF: CT14
 Obs4: Free: Sig0, A1, A2, B  -> Fixados no ajuste de BSGorig_band2str2_V001.C
 ...
 *** Bandas de incerteza ***
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_band2str2_V001.C | cat > BSG20orig_SigEl_band2str2_V001.min
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_band2str2_V001.eps");    
}
//----------------------------------------------------------------------
...
void BSG20orig_SigEl_band2str2_V001()
{
    /******************************************/
    //time control - start
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    //fit parameters
    double ww=1.0;//outpar[0]; 
    double Sig0=6.96838e+01;//outpar[0];
    double A1=1.92068e+01;//outpar[1];
    double del1=0.5;//outpar[3];
    double A2=1.75152e+01;//outpar[2];  
    double del2=0.5;//outpar[5];
    double B=1.5;//outpar[3];
    /*
    1  Sig0         6.96838e+01   2.76345e-01  -7.54798e-10  -8.57124e-10
    2  A1           1.92068e+01   6.44932e-01   2.26136e-09   6.08670e-10
    3  A2           1.75152e+01   3.58456e-01   6.60376e-10  -1.01445e-09
    */    
    /*******************************************************/
    //Total XSection - pp
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_band2str2_V001.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_band2str2_V001.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_band2str2_V001.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_band2str2_V001.dat","r");
