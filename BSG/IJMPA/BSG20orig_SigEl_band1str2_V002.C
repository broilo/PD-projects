...
------------------------------------------------------------------------
 ...
 Obs3: PDF: CT14
 Obs4: Free: w, Sig0, A1, A2 -> Fixados no ajuste de BSGorig_band1str2_V002.C
 ...
 *** Bandas de incerteza ***
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_band1str2_V002.C | cat > BSG20orig_SigEl_band1str2_V002.min
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_band1str2_V002.eps");    
}
//----------------------------------------------------------------------
...
void BSG20orig_SigEl_band1str2_V002()
{
    /******************************************/
    //time control - start
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    /*******************************************************/ 
    //fit parameters
    double ww=0.8;//outpar[0]; 
    double Sig0=8.82801e+01;//outpar[0];
    double A1=1.40668e+01;//outpar[1];
    double del1=0.5;//outpar[3];
    double A2=1.99701e+01;//outpar[2];  
    double del2=0.5;//outpar[5];
    double B=0.76;//outpar[3];
    /*
   1  Sig0         8.82801e+01   3.22633e-01  -5.54341e-08  -6.41680e-09
   2  A1           1.40668e+01   7.49250e-01   1.37653e-07   4.19859e-09
   3  A2           1.99701e+01   4.13123e-01   2.65346e-08  -4.97885e-09
    */    
    /*******************************************************/
    //Total XSection - pp
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_band1str2_V002.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_band1str2_V002.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_band1str2_V002.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_band1str2_V002.dat","r");
