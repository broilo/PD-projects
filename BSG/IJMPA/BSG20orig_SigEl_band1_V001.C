...
------------------------------------------------------------------------
 ...
 Obs3: PDF: CT14
 Obs4: Free: Sig0, A1, A2, B -> Fixados no ajuste de BSGorig_V001.C
 ...
 *** Bandas de incerteza ***
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_band1_V001.C | cat > BSG20orig_SigEl_band1_V001.min
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_band1_V001.eps");    
}
//----------------------------------------------------------------------
...
void BSG20orig_SigEl_band1_V001()
{
    /******************************************/
    //time control - start
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    //fit parameters
    double ww=1.0;//outpar[0]; 
    double Sig0=7.35489e+01;//outpar[0];
    double A1=1.30225e+01;//outpar[1];
    double del1=0.5;//outpar[3];
    double A2=1.72119e+01;//outpar[2];  
    double del2=0.5;//outpar[5];
    double B=0.5;//outpar[3];
    /*
    1  Sig0         7.35489e+01   3.46423e-01  -2.00851e-05   5.12825e-07
    2  A1           1.30225e+01   7.75315e-01   5.91104e-05  -1.00426e-07
    3  A2           1.72119e+01   4.11404e-01  -4.82855e-06   3.57866e-10
    4  B            7.55541e-01   2.37580e-02   5.45726e-06  -1.17997e-06
    */    
    /*******************************************************/
    //Total XSection - pp
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_band1_V001.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_band1_V001.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_band1_V001.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_band1_V001.dat","r");
