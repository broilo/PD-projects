...
------------------------------------------------------------------------
 ...
 Obs3: PDF: CT14
 Obs4: Free: w, Sig0, A1, A2 -> Fixados no ajuste de BSGorig_w002.C
 ...
 root -l -q /usr/lib/x86_64-linux-gnu/libgsl.so /usr/lib/x86_64-linux-gnu/libgslcblas.so BSG20orig_SigEl_w002.C | cat > BSG20orig_SigEl_w002.min
...
/* ---> desenhando um miniframe em um pad dentro do canvas canv1 <--- */
    ...
    grSigPP_Eik->Draw("csame");
      ...
      canv1->SaveAs("SigEl_BSG20orig_SigEl_w002.eps");    
}
//----------------------------------------------------------------------
...
{    
   /*******************************************************/ 
    //fit parameters
    double ww=2.58673e-01;//outpar[0]; 
    double Sig0=2.43860e+02;//outpar[0];
    double A1=6.50488e+01;//outpar[1];
    double del1=1.51370e+00;//outpar[3];
    double A2=1.69386e+01;//outpar[2];  
    double del2=5.70379e+01;//outpar[5];
    double B=5.73657e-01;//outpar[3];
    /*******************************************************/
...
//    gMinuit->mnexcm("MIGRAD", arglist ,1,ierflg);   
  ...
  cout << " Status of covariance matrix = " << icstat << "\n\n";  
   ...
   for (int i=0; i<numpar; i++)
    ...
    double A1=6.50488e+01;//outpar[1];
    double del1=1.51370e+00;//outpar[3];
    double A2=5.70379e+01;//outpar[2];  
    double del2=5.73657e-01;//outpar[5];
    double B=2.0;//outpar[3];
    /*
   1  w            2.58673e-01   1.13700e-02  -8.69980e-07  -3.24488e-04
   2  Sig0         2.43860e+02   1.03154e+01   9.33502e-04  -5.95548e-06
   3  A1           6.50488e+01   6.08376e+00   9.97208e-05  -3.25623e-06
   4  del1         1.51370e+00   1.18796e-01   2.68251e-06   1.99194e-04
   5  A2           5.70379e+01   3.99074e+00   8.35954e-05   6.41982e-07
   6  del2         5.73657e-01   4.26905e-02  -1.40483e-06   1.15971e-05  
    */
    /*******************************************************/
    //Total XSection - pp
...
//----------------------------------------------------------------------
   //calculates total xsection and rho, creating files to make the plots
    ...
    aq1=fopen("SigElpp_BSG20orig_SigEl_w002.dat","w"); 
    aq2=fopen("SigElpbp_BSG20orig_SigEl_w002.dat","w"); 
    ...
    double sigpp,sigpbarp;
     ...
     do{
      ...
      aq1 = fopen("SigElpp_BSG20orig_SigEl_w002.dat","r");
      aq2 = fopen("SigElpbp_BSG20orig_SigEl_w002.dat","r");
